...
// here on purpose: the UI never supplies install commands, so setup buttons
// cannot turn a tool id into an arbitrary remote command by accident.
...
// SetupOverview is the reply to a setup status probe.
...
func SetupStatus(parent context.Context, c Conn, osName, arch string) SetupOverview {
	...
	for i, spec := range setupSpecs {
		...
		go func(i int, spec toolSpec) {
			defer wg.Done()
			tools[i] = probeSetupTool(parent, c, osName, spec)
		}(i, spec)
	}
	wg.Wait()
	...
	return SetupOverview{OS: osName, Arch: arch, Tools: tools, BundleCommand: strings.Join(installs, separator)}
}

func probeSetupTool(parent context.Context, c Conn, osName string, spec toolSpec) SetupTool {
	tool := SetupTool{
	...
	res, err := Exec(parent, c, command, "", 15)
	if err != nil {
		tool.Auth = "unknown"
		return tool
	}
	...
	if spec.authProbe != "" {
		if !tool.Installed {
			...
			authRes, authErr := Exec(parent, c, authCommand, "", 4)
