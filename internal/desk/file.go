...
func deskConfigPath(env map[string]string) (string, error) {
	if explicit := strings.TrimSpace(env[brand.EnvDeskConfig]); explicit != "" {
		if _, err := os.Stat(explicit); err != nil {
			return "", fmt.Errorf("%w: %s is %q, and that file cannot be read: %v",
				ErrConfig, brand.EnvDeskConfig, explicit, err)
		}
		return explicit, nil
	}
	...
	fallback := filepath.Join(home, brand.ConfigDir, brand.DeskConfigFile)
...
func deskFileToEnv(raw []byte) (map[string]string, error) {
	...
	if file.Seam != nil {
		put(env, brand.EnvDeskSeamAddr, file.Seam.Addr)
		put(env, brand.EnvDeskSeamToken, file.Seam.Token)
	}
	if file.Roles != nil {
		put(env, brand.EnvDeskChat, file.Roles.Chat)
		put(env, brand.EnvDeskSTT, file.Roles.STT)
		put(env, brand.EnvDeskTTS, file.Roles.TTS)
	}
	for i, provider := range file.Providers {
		...
		prefix := brand.EnvDeskProviderPrefix + segment
