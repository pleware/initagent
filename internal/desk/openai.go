...
var ErrDialect = errors.New("desk: dialect")
...
var ErrRequest = errors.New("desk: request")
...
func (e *ProviderError) Error() string {
	if e.Code == 0 {
		return fmt.Sprintf("desk: provider %s failed mid-reply: %s", e.Provider, e.Message)
	}
	return fmt.Sprintf("desk: provider %s refused (%d): %s", e.Provider, e.Code, e.Message)
}
	...
	return func(yield func(ChatDelta, error) bool) {
		...
		for payload, err := range sseData(resp.Body) {
			...
			if err := json.Unmarshal(payload, &chunk); err != nil {
				yield(ChatDelta{}, fmt.Errorf("desk: provider %s sent an unreadable event: %w", provider, err))
				return
			}
			if chunk.Error != nil {
...
func sseData(r io.Reader) iter.Seq2[[]byte, error] {
	return func(yield func([]byte, error) bool) {
		...
		if err := scanner.Err(); err != nil {
			...
			yield(nil, fmt.Errorf("desk: reading reply: %w", err))
		}
	}
}
