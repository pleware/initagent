...
// desk.Facts, so the runner writes into it directly rather than through an
...
type Delivery struct {
	mu      sync.Mutex
	log     *Log
	names   map[desk.StaffID]string
	unclear string
	replies map[desk.UtteranceID]CommandID
	spoken  []desk.UtteranceID
}
...
type DeliveryConfig struct {
	...
	// her name. Required.
	//
	// Deliberately not desk.Roster: the roster holds *folded* forms, lowercase
...
func NewDelivery(cfg DeliveryConfig) (*Delivery, error) {
	...
	return &Delivery{
		...
		unclear: unclear,
		replies: make(map[desk.UtteranceID]CommandID),
	}, nil
}
...
// them.
func (d *Delivery) Attribute(utterance desk.UtteranceID, cmd CommandID) {
	if utterance == "" {
		return
	}
	d.mu.Lock()
...
func (d *Delivery) Record(fact desk.Fact) {
	switch got := fact.(type) {
	case desk.TurnOpened:
		d.open(got)
	case desk.Said:
		d.say(got)
	case desk.AddressingUnclear:
		d.ask(got)
	case desk.Failed:
		d.failed(got)
	}
	// desk.FloorMoved stays here. Who holds the floor is the connector's
...
func (d *Delivery) open(fact desk.TurnOpened) {
...
func (d *Delivery) say(fact desk.Said) {
	id := surfaceForTurn(fact.Turn)
	if fact.Final {
...
func (d *Delivery) ask(fact desk.AddressingUnclear) {
...
func (d *Delivery) replyTo(utterance desk.UtteranceID) CommandID {
	d.mu.Lock()
	defer d.mu.Unlock()
	return d.replies[utterance]
}

func surfaceForTurn(turn desk.TurnID) SurfaceID {
