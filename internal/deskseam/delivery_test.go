...
func testNames() map[desk.StaffID]string {
	return map[desk.StaffID]string{"psn-ania": "Ania", "psn-adam": "Adam"}
}
...
func TestNewDeliveryRefusesADeliveryThatCannotName(t *testing.T) {
	for _, tc := range []struct {
		...
		{"a blank name", DeliveryConfig{Log: newTestLog(t, 0, nil), Names: map[desk.StaffID]string{"psn-ania": " "}}, "no name to be shown by"},
...
func TestRecordOpensASurfaceInHerName(t *testing.T) {
	...
	delivery.Record(desk.TurnOpened{Turn: "trn-1", Staff: "psn-ania", Utterance: "utt-1", Text: "sprawdź to"})
...
func TestRecordAppendsEachPieceSheSays(t *testing.T) {
	delivery, log := newTestDelivery(t)
	delivery.Record(desk.Said{Turn: "trn-1", Staff: "psn-ania", Text: "sprawdzam"})
	delivery.Record(desk.Said{Turn: "trn-1", Staff: "psn-ania", Text: " opony"})
...
func TestRecordClosesTheSlotWithoutRepeatingHerLastWords(t *testing.T) {
	delivery, log := newTestDelivery(t)
	delivery.Record(desk.Said{Turn: "trn-1", Staff: "psn-ania", Text: "gotowe"})
	delivery.Record(desk.Said{Turn: "trn-1", Staff: "psn-ania", Final: true})
...
func TestRecordSplitsALineTooLongForTheGlass(t *testing.T) {
	delivery, log := newTestDelivery(t)
	delivery.Record(desk.Said{Turn: "trn-1", Text: strings.Repeat("a", MaxChunkChars+1)})
...
func TestRecordAsksWhoWasMeant(t *testing.T) {
	delivery, log := newTestDelivery(t)
	delivery.Record(desk.AddressingUnclear{
		Utterance:  "utt-1",
		Candidates: []desk.StaffID{"psn-ania", "psn-adam"},
		Text:       "sprawdź to",
	})
...
func TestRecordTurnsTheSurfaceIntoTheSeamsFailureView(t *testing.T) {
	delivery, log := newTestDelivery(t)
	delivery.Record(desk.Failed{
		Turn:    "trn-1",
		Failure: desk.Failure{Code: desk.FailureUpstreamFailed, Message: "provider said no"},
	})
...
func TestRecordSaysNothingAboutTheFloor(t *testing.T) {
	delivery, log := newTestDelivery(t)
	delivery.Record(desk.FloorMoved{Staff: "psn-adam", Reason: desk.FloorNamed})
...
func TestRecordAnswersTheCommandTheUtteranceArrivedOn(t *testing.T) {
	...
	delivery.Record(desk.TurnOpened{Turn: "trn-1", Staff: "psn-ania", Utterance: "utt-1"})
	delivery.Record(desk.Said{Turn: "trn-1", Text: "robi się"})
...
func TestRecordCarriesNoAnswerForAnUtteranceItWasNotToldAbout(t *testing.T) {
	delivery, log := newTestDelivery(t)
	delivery.Record(desk.TurnOpened{Turn: "trn-1", Staff: "psn-ania", Utterance: "utt-9"})
...
func TestAttributeForgetsTheOldestPastItsCeiling(t *testing.T) {
	...
	for i := range maxAttributions + 1 {
		delivery.Attribute(desk.UtteranceID(fmt.Sprintf("utt-%d", i)), CommandID(fmt.Sprintf("cmd-%d", i)))
	}
...
func TestRecordLabelsAStrangerWithWhatItHas(t *testing.T) {
	delivery, log := newTestDelivery(t)
	delivery.Record(desk.TurnOpened{Turn: "trn-1", Staff: "psn-nobody"})
...
func TestRecordIgnoresAFactItHasNoSurfaceFor(t *testing.T) {
	delivery, log := newTestDelivery(t)
	delivery.Record(desk.FloorMoved{Staff: "psn-ania"})
	delivery.Record(desk.Said{Turn: "trn-1", Text: "cześć"})
