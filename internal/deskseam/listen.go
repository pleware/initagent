...
// Path is where the desk seam is mounted on the connector's local address.
...
const (
	...
	// desk.MaxUtteranceChars; this is that plus room for an envelope, so a
...
type ListenConfig struct {
	...
	Views *Views
	// Answerer runs a turn. *desk.Runner is one.
	Answerer Answerer
...
func (l *Listener) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if stream == "" {
		http.Error(w, "desk: stream required", http.StatusBadRequest)
		return
	}
...
func Talk(ws *websocket.Conn, socket *Socket, view *View) {
	...
	turns := make(chan desk.Utterance, MaxTurnsWaiting)
...
func answerTurns(ctx context.Context, socket *Socket, turns <-chan desk.Utterance) {
	for {
		select {
		case <-ctx.Done():
			return
