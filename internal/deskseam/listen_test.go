...
func (a *echoAnswerer) Answer(_ context.Context, spoken desk.Utterance) error {
	a.mu.Lock()
	a.heard = append(a.heard, spoken)
	a.mu.Unlock()

	a.views.Record(spoken.Conversation, desk.TurnOpened{
		Turn:      "trn-1",
		Staff:     "psn-ania",
		Utterance: spoken.ID,
	})
...
func (a *echoAnswerer) spoken() []desk.Utterance {
	a.mu.Lock()
	defer a.mu.Unlock()
	return append([]desk.Utterance(nil), a.heard...)
}

// newTestListener serves one desk over a real socket.
func newTestListener(t *testing.T, conv desk.ConversationID) (*httptest.Server, *Views, *echoAnswerer) {
	t.Helper()
...
func TestAConnectionWithoutTheTokenIsRefusedBeforeTheUpgrade(t *testing.T) {
	...
	for _, token := range []string{"", "sec-wrong"} {
		_, resp, err := websocket.DefaultDialer.Dial(socketURL(server, "desk:local", token), nil)
		if err == nil {
...
func TestTheTokenMayArriveInAHeaderToo(t *testing.T) {
	...
	ws, resp, err := websocket.DefaultDialer.Dial(socketURL(server, "desk:local", ""), header)
	if err != nil {
		t.Fatalf("dial: %v (status %v)", err, statusOf(resp))
	}
	ws.Close()
...
func TestAStreamSomebodyElseIsOnIsRefusedWithAStatus(t *testing.T) {
	server, views, _ := newTestListener(t, "person")
	if _, err := views.Bind("desk:local", "somebody-else"); err != nil {
		t.Fatal(err)
	}

	_, resp, err := websocket.DefaultDialer.Dial(socketURL(server, "desk:local", testToken), nil)
	if err == nil {
		t.Fatal("want the dial refused")
	}
...
func TestAResyncReplaysToThisConnectionOnly(t *testing.T) {
	server, views, _ := newTestListener(t, "person")
	first := dial(t, server, "desk:phone", testToken)

	say(t, first, "desk:phone", "cmd-1", "utt-1", "cześć")
	...
	second := dial(t, server, "desk:glass", testToken)
	raw := frame(t, "desk:glass", "cmd-2", CommandResync, map[string]any{"fromSeq": 0})
	...
	views.Record("person", desk.TurnOpened{Turn: "trn-9", Staff: "psn-adam", Utterance: "utt-9"})
...
func TestACommandForAnotherStreamIsRefusedOnTheWire(t *testing.T) {
	...
	say(t, ws, "desk:somebody-else", "cmd-9", "utt-1", "cześć")
...
func TestReconnectingWithTheSameStreamKeepsWhatSheAlreadyHeard(t *testing.T) {
	...
	first := dial(t, server, "desk:local", testToken)
	say(t, first, "desk:local", "cmd-1", "utt-1", "cześć")
	readEvent(t, first)
	first.Close()

	second := dial(t, server, "desk:local", testToken)
	raw := frame(t, "desk:local", "cmd-2", CommandResync, map[string]any{"fromSeq": 0})
	if err := second.WriteMessage(websocket.TextMessage, raw); err != nil {
		t.Fatalf("write: %v", err)
	}
...
func TestAFactTheConnectorCannotEncodeCostsTheConnectionNothing(t *testing.T) {
	...
	view, err := views.Bind("desk:local", "person")
	if err != nil {
		t.Fatal(err)
	}
...
func TestATurnThatFailsLeavesTheConnectionOpen(t *testing.T) {
	...
	ws := dial(t, server, "desk:local", testToken)
	say(t, ws, "desk:local", "cmd-1", "utt-1", "cześć")
	...
	views.Record("person", desk.TurnOpened{Turn: "trn-2", Staff: "psn-ania", Utterance: "utt-2"})
