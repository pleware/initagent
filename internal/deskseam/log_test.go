...
func newTestLog(t *testing.T, max int, notify func(Event)) *Log {
	t.Helper()
	log, err := NewLog(LogConfig{Stream: "desk:local", Max: max, Now: fixedClock(), Notify: notify})
	if err != nil {
		t.Fatal(err)
	}
	return log
...
func TestNewLogDefaultsTheCeilingAndTheClock(t *testing.T) {
	log, err := NewLog(LogConfig{Stream: "desk:local"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestAppendStampsTheEnvelope(t *testing.T) {
	...
	if got.V != Version || got.Stream != "desk:local" || got.Kind != EventSurfaceOpened {
