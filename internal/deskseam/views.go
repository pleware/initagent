...
func (v *Views) Bind(stream StreamID, conv desk.ConversationID) (*View, error) {
	if stream == "" {
...
func (v *Views) open(stream StreamID, conv desk.ConversationID) (*View, error) {
	feed := NewFeed()
...
//
// It implements desk.Facts, so the runner writes here and this package decides
...
func (v *Views) Record(conv desk.ConversationID, fact desk.Fact) {
...
func (v *Views) watching(conv desk.ConversationID) []*View {
	v.mu.Lock()
	defer v.mu.Unlock()
...
func (v *View) Conversation() desk.ConversationID { return v.conv }
