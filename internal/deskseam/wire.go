// Package deskseam carries desk facts to the glass over the seam the glass
// already speaks.
//
...
var ErrSeam = errors.New("desk seam")
...
const (
	CommandUtterance = "desk.utterance"
...
func parseUtterance(payload json.RawMessage) (desk.Utterance, error) {
	var got wireUtterance
	if err := json.Unmarshal(payload, &got); err != nil {
		return desk.Utterance{}, fmt.Errorf("%w: utterance payload is not an object", ErrSeam)
	}
	if got.UtteranceID == "" {
		return desk.Utterance{}, fmt.Errorf("%w: utterance has no id", ErrSeam)
	}
	if strings.TrimSpace(got.Text) == "" {
		return desk.Utterance{}, fmt.Errorf("%w: utterance has no words", ErrSeam)
	}
	if utf8.RuneCountInString(got.Text) > desk.MaxUtteranceChars {
		return desk.Utterance{}, fmt.Errorf("%w: utterance is longer than %d characters", ErrSeam, desk.MaxUtteranceChars)
	}
	source := desk.UtteranceSource(got.Source)
	if source != desk.UtteranceVoice && source != desk.UtteranceTyped {
		return desk.Utterance{}, fmt.Errorf("%w: utterance source %q is not spoken or typed", ErrSeam, got.Source)
	}
	return desk.Utterance{
		ID:     desk.UtteranceID(got.UtteranceID),
		Text:   got.Text,
		Source: source,
	}, nil
}
