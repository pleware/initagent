...
func TestParseCommandClassifiesWhatCannotBeACommand(t *testing.T) {
	for _, tc := range []struct {
		...
		{"another envelope version", `{"v":2,"cmdId":"c1","stream":"desk:local","kind":"desk.utterance","payload":{}}`, InboundMalformed, "bad command"},
		{"no command id", `{"v":1,"cmdId":"","stream":"desk:local","kind":"desk.utterance","payload":{}}`, InboundMalformed, "bad command"},
		{"no stream", `{"v":1,"cmdId":"c1","stream":"","kind":"desk.utterance","payload":{}}`, InboundMalformed, "bad command"},
		{"no verb", `{"v":1,"cmdId":"c1","stream":"desk:local","kind":"","payload":{}}`, InboundMalformed, "bad command"},
	} {
...
func TestParseCommandKeepsAVerbItDoesNotAnswer(t *testing.T) {
	got := ParseCommand([]byte(`{"v":1,"cmdId":"c1","stream":"desk:local","kind":"catalog.open","payload":{"path":[]}}`))
...
func TestParseCommandReadsAnUtterance(t *testing.T) {
	got := ParseCommand([]byte(`{"v":1,"cmdId":"c1","stream":"desk:local","kind":"desk.utterance","payload":{"utteranceId":"utt-1","text":"ania sprawdź to","source":"typed"}}`))
	if got.Kind != InboundCommand {
		t.Fatalf("kind = %q, detail = %q", got.Kind, got.Detail)
	}
	want := desk.Utterance{ID: "utt-1", Text: "ania sprawdź to", Source: desk.UtteranceTyped}
	if got.Spoken != want {
...
func TestParseCommandRefusesAnUtteranceNobodyCanAct(t *testing.T) {
	for _, tc := range []struct {
		...
		{"too long", `{"utteranceId":"utt-1","text":"` + strings.Repeat("a", desk.MaxUtteranceChars+1) + `","source":"typed"}`, "utterance is longer than"},
		{"unknown source", `{"utteranceId":"utt-1","text":"cześć","source":"telepathy"}`, "is not spoken or typed"},
	} {
		t.Run(tc.name, func(t *testing.T) {
			got := ParseCommand([]byte(`{"v":1,"cmdId":"c1","stream":"desk:local","kind":"desk.utterance","payload":` + tc.payload + `}`))
...
func TestParseCommandReadsAResync(t *testing.T) {
	got := ParseCommand([]byte(`{"v":1,"cmdId":"c1","stream":"desk:local","kind":"stream.resync","payload":{"fromSeq":7}}`))
...
func TestParseCommandReadsAResyncFromTheBeginning(t *testing.T) {
	got := ParseCommand([]byte(`{"v":1,"cmdId":"c1","stream":"desk:local","kind":"stream.resync","payload":{"fromSeq":0}}`))
...
func TestParseCommandRefusesAResyncItCannotHonour(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			got := ParseCommand([]byte(`{"v":1,"cmdId":"c1","stream":"desk:local","kind":"stream.resync","payload":` + tc.payload + `}`))
			if got.Kind != InboundInvalid {
				t.Fatalf("kind = %q, want %q", got.Kind, InboundInvalid)
			}
			if !strings.Contains(got.Detail, tc.detail) {
...
func TestEncodeWritesTheEnvelopeAndPayloadInOneObject(t *testing.T) {
	...
	if got["v"] != float64(1) || got["seq"] != float64(3) || got["kind"] != EventSurfaceAppended {
		t.Fatalf("envelope = %+v", got)
	}
	if got["inReplyTo"] != "c1" || got["stream"] != "desk:local" || got["at"] != "2026-09-11T03:00:00Z" {
		t.Fatalf("envelope = %+v", got)
	}
