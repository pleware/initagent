...
// speaks as her.
//
// A struct here rather than desk.Staff plus two maps, because these four
...
type Person struct {
	...
	// routes work to the wrong person silently (desk.Staff).
	Names     []string
	Vocatives []string
...
//
// Two people, fixed at desk.MaxStaff, and both are ours: talking to the desk
...
func rosterOf(staff []Person) []desk.Staff {
	out := make([]desk.Staff, 0, len(staff))
	for _, person := range staff {
		out = append(out, desk.Staff{
			ID:        person.ID,
			Names:     person.Names,
...
func namesOf(staff []Person) map[desk.StaffID]string {
	out := make(map[desk.StaffID]string, len(staff))
	for _, person := range staff {
		name := person.Display
		if name == "" {
...
// hub's copy advisory.
type personas map[desk.StaffID]desk.Persona
...
func personasOf(staff []Person) personas {
	...
	for _, person := range staff {
		...
		out[person.ID] = desk.Persona{Brief: person.Brief, MaxWords: words}
	}
	return out
}

// Persona implements desk.Personas.
func (p personas) Persona(_ context.Context, staff desk.StaffID) (desk.Persona, error) {
	persona, ok := p[staff]
	if !ok {
		return desk.Persona{}, fmt.Errorf("%w: nobody at this desk is %q", desk.ErrConfig, staff)
	}
	return persona, nil
}
