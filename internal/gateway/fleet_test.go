...
func TestFleetAgentsListsSessionsAcrossDevices(t *testing.T) {
	...
	if rows[0].Name != "term-1" || rows[0].DeviceId != deviceID || rows[0].DeviceName != "box" {
...
func TestFleetAgentsEmptyWhenOffline(t *testing.T) {
	g := openTest(t, "")
	if _, _, err := g.Store().CreateDevice(t.Context(), g.Project().ID, "box", "box", "linux", "amd64"); err != nil {
		t.Fatal(err)
	}
	req := httptest.NewRequest(http.MethodGet, "/api/agents", nil)
	rec := httptest.NewRecorder()
