...
func (s *Store) Enqueue(ctx context.Context, task scheduler.Task) (scheduler.Task, error) {
	...
	task.CreatedAt = now
	task.UpdatedAt = now
	mode, err := scheduler.NormalizeLaunch(task.LaunchMode)
	if err != nil {
		return scheduler.Task{}, err
	}
	...
	_, err = s.db.ExecContext(ctx, `
		INSERT INTO tasks (
			id, project_id, requester_id, executor_id, state, coder_kind,
			assigned_worker_id, lease_expiry, created_at, updated_at,
			exit_code, reason, command, launch_mode
		) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
	`, task.ID, task.ProjectID, task.RequesterID, task.ExecutorID, string(task.State),
...
const taskSelect = `
SELECT id, project_id, requester_id, executor_id, state, coder_kind,
...
func scanTask(row rowScanner) (scheduler.Task, error) {
	...
	err := row.Scan(
		&t.ID, &t.ProjectID, &t.RequesterID, &t.ExecutorID, &state, &t.CoderKind,
...
func (s *Store) Claim(ctx context.Context, projectID, workerID string, lease time.Duration) (*scheduler.Task, *scheduler.Lease, error) {
	...
	// the oldest queued row for this project; requester_id is stored, not used
	// as a placement key.
