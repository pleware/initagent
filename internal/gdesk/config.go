...
var ErrConfig = errors.New("gdesk: configuration")
...
var roleEnv = map[Role]string{
	RoleChat: brand.EnvGdeskChat,
	RoleSTT:  brand.EnvGdeskSTT,
	RoleTTS:  brand.EnvGdeskTTS,
}
...
func parseSeam(env map[string]string) (Seam, error) {
	seam := Seam{
		Addr:  strings.TrimSpace(env[brand.EnvGdeskSeamAddr]),
		Token: strings.TrimSpace(env[brand.EnvGdeskSeamToken]),
	}
	if seam.Addr == "" {
	...
	if err != nil || port == "" {
		return Seam{}, fmt.Errorf("%w: %s must be host:port, got %q", ErrConfig, brand.EnvGdeskSeamAddr, seam.Addr)
	}
	if !loopback(host) {
		return Seam{}, fmt.Errorf("%w: %s is %q, and the desk seam listens on loopback only — a device off this box joins through the hub",
			ErrConfig, brand.EnvGdeskSeamAddr, seam.Addr)
	}
	return seam, nil
}
...
func bindRole(role Role, spec string, providers map[string]Provider, env map[string]string) (Binding, error) {
	...
	if !ok {
		return Binding{}, fmt.Errorf("%w: %s names provider %q, which no %s%s_SHAPE declares",
			ErrConfig, roleEnv[role], name, brand.EnvGdeskProviderPrefix, envSegment(name))
	}
...
func parseProviders(env map[string]string) (map[string]Provider, error) {
	...
	for _, key := range slices.Sorted(maps.Keys(env)) {
		rest, ok := strings.CutPrefix(key, brand.EnvGdeskProviderPrefix)
		if !ok {
			continue
		}
	...
	if _, ok := shapeBaseURL[p.Shape]; !ok {
		if p.Shape == "" {
			return fmt.Errorf("%w: provider %q needs %s%s_SHAPE",
				ErrConfig, p.ID, brand.EnvGdeskProviderPrefix, envSegment(p.ID))
		}
...
func rejectUnknownKeys(env map[string]string) error {
	...
	var unknown []string
	for key := range env {
		if !strings.HasPrefix(key, brand.GdeskEnvPrefix) || known[key] {
			continue
		}
		if rest, ok := strings.CutPrefix(key, brand.EnvGdeskProviderPrefix); ok && knownProviderField(rest) {
			continue
		}
