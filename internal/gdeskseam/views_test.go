...
func TestBindingTheSameStreamTwiceKeepsItsHistory(t *testing.T) {
	...
	first, err := views.Bind("gdesk:local", "phone")
	if err != nil {
		t.Fatalf("Bind: %v", err)
	}
	...
	again, err := views.Bind("gdesk:local", "phone")
	if err != nil {
