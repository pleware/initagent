...
func TestCreateSessionOfflineWithoutGateway(t *testing.T) {
	srv := newTestServer(t, "v0.1.0")
	req := httptest.NewRequest(http.MethodPost, "/api/devices/dev-01/sessions", strings.NewReader(`{"name":"claude-1"}`))
	req.Header.Set("Content-Type", "application/json")
	req.SetPathValue("id", "dev-01")
	w := httptest.NewRecorder()
...
func TestCreateSessionAsksGateway(t *testing.T) {
	...
	if gotMethod != http.MethodPost || gotPath != "/api/devices/dev-01/sessions" {
...
func TestListSessionsAsksGateway(t *testing.T) {
	gw := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Method != http.MethodGet || r.URL.Path != "/api/devices/dev-01/sessions" {
	...
	srv := newTestServer(t, "v0.1.0")
	srv.opts.GatewayURL = gw.URL
	req := httptest.NewRequest(http.MethodGet, "/api/devices/dev-01/sessions", nil)
	req.SetPathValue("id", "dev-01")
	w := httptest.NewRecorder()
...
func projectWithDevice(t *testing.T, srv *Server) *Project {
	...
	project, err := srv.store.CreateProject(org.Id, "Storefront", "dev-01", "/srv/store", "", "", "", "")
	if err != nil {
		t.Fatal(err)
	}
	return project
...
func TestProjectExecAsksGateway(t *testing.T) {
	...
	if gotMethod != http.MethodPost || gotPath != "/api/devices/dev-01/exec" {
		t.Fatalf("proxied %s %s", gotMethod, gotPath)
	}
...
func TestFsListOfflineWithoutGateway(t *testing.T) {
	srv := newTestServer(t, "v0.1.0")
	req := httptest.NewRequest(http.MethodGet, "/api/devices/dev-01/fs", nil)
	req.SetPathValue("id", "dev-01")
	w := httptest.NewRecorder()
...
func TestFsListAsksGateway(t *testing.T) {
	...
	srv := newTestServer(t, "v0.1.0")
	srv.opts.GatewayURL = gw.URL
	req := httptest.NewRequest(http.MethodGet, "/api/devices/dev-01/fs?path=/home", nil)
	req.SetPathValue("id", "dev-01")
	w := httptest.NewRecorder()
	srv.handleFsList(w, req, operatorCred)
	if w.Code != http.StatusOK {
	...
	if gotPath != "/api/devices/dev-01/fs?path=/home" {
...
func TestFsDownloadAsksGateway(t *testing.T) {
	gw := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/api/devices/dev-01/fs/download" {
	...
	srv := newTestServer(t, "v0.1.0")
	srv.opts.GatewayURL = gw.URL
	req := httptest.NewRequest(http.MethodGet, "/api/devices/dev-01/fs/download?path=/srv/report.txt", nil)
	req.SetPathValue("id", "dev-01")
	w := httptest.NewRecorder()
...
func TestFsUploadAsksGateway(t *testing.T) {
	...
	_ = mw.Close()
	req := httptest.NewRequest(http.MethodPost, "/api/devices/dev-01/fs/upload?dir=/srv", &buf)
	...
	if gotMethod != http.MethodPost || gotPath != "/api/devices/dev-01/fs/upload?dir=/srv" {
		t.Fatalf("proxied %s %s", gotMethod, gotPath)
	}
	if gotForm != "uploaded" {
...
func TestSetupStatusAsksGateway(t *testing.T) {
	gw := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/api/devices/dev-01/setup" {
			http.Error(w, "unexpected", http.StatusBadRequest)
			return
		}
		w.Header().Set("Content-Type", "application/json")
	...
	srv := newTestServer(t, "v0.1.0")
	srv.opts.GatewayURL = gw.URL
	req := httptest.NewRequest(http.MethodGet, "/api/devices/dev-01/setup", nil)
	req.SetPathValue("id", "dev-01")
	w := httptest.NewRecorder()
