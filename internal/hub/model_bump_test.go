...
func TestModelWriteBumpsAllBoxes(t *testing.T) {
	...
	m, err := s.CreateModel("bump-worker", "o", "s", "", "", "bump-digest", "MIT", []string{"worker"})
	if err != nil {
		t.Fatal(err)
	}
	...
	before = boxVersions(t, s, boxes...)
	if _, err := s.UpdateModel(m.ID, "o", "s2", "q4_0", "", "bump-digest", "MIT", []string{"worker"}); err != nil {
		t.Fatal(err)
	}
	assertBumpDelta(t, before, boxVersions(t, s, boxes...), both)
	...
	before = boxVersions(t, s, boxes...)
	if _, err := s.UpdateModel("no-such-model", "o", "s2", "", "", "", "MIT", []string{"worker"}); err != nil {
		t.Fatal(err)
	}
